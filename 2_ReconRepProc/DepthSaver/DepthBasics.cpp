...
void CDepthBasics::ProcessDepth()
{
	...
	if (LockedRect.Pitch != 0)
	{
		...
		int minDepth = 700;//(nearMode ? NUI_IMAGE_DEPTH_MINIMUM_NEAR_MODE : NUI_IMAGE_DEPTH_MINIMUM) >> NUI_IMAGE_PLAYER_INDEX_SHIFT;
		int maxDepth = 920;//(nearMode ? NUI_IMAGE_DEPTH_MAXIMUM_NEAR_MODE : NUI_IMAGE_DEPTH_MAXIMUM) >> NUI_IMAGE_PLAYER_INDEX_SHIFT;
		...
		//Write Bitmap se a flag estiver settada, tá em BGR e eu estou escrevendo RGB
		//Mas não deve fazer diferenca uma vez que são tons de cinza
		if (this->saveFrame) {
			char fileName[255]; // enough to hold all numbers up to 64-bits
			sprintf(fileName, "depthImage%d.bmp", saveNumber++);
			SOIL_save_image(fileName, SOIL_SAVE_TYPE_BMP, 640, 480, 4, m_depthRGBX);
			SetStatusMessage(L"Depth Image Saved!");
