...
addForces_k(cData *v, GLint dx, GLint dy, GLint spx, GLint spy, GLfloat fx, GLfloat fy, GLint r, size_t pitch)
{
	...
	GLint tx = threadIdx.x;
	GLint ty = threadIdx.y;
	cData *fj = (cData *)((GLchar *)v + (ty + spy) * pitch) + tx + spx;

	cData vterm = *fj;
	tx -= r;
	ty -= r;
	GLfloat s = 1.f / (1.f + tx*tx*tx*tx + ty*ty*ty*ty);
	vterm.x += s * fx;
	vterm.y += s * fy;
	*fj = vterm;
}
...
GLint dx, GLint pdx, GLint dy, GLfloat dt, GLint lb)
{
	...
	GLint gtidx = blockIdx.x * blockDim.x + threadIdx.x;
	GLint gtidy = blockIdx.y * (lb * blockDim.y) + threadIdx.y * lb;
	GLint p;

	cData vterm, ploc;
	GLfloat vxterm, vyterm;
	...
	if (gtidx < dx)
	{
		for (p = 0; p < lb; p++)
		{
			...
			if (fi < dy)
			{
				GLint fj = fi * pdx + gtidx;
				vterm = tex2D(texref, (GLfloat)gtidx, (GLfloat)fi);
	...
	if (gtidx < dx)
	{
		for (p = 0; p < lb; p++)
		{
			...
			if (fi < dy)
			{
				...
				GLint iix = gtidx;
				GLint iiy = (fi>dy / 2) ? (fi - (dy)) : fi;

				// Velocity diffusion
				GLfloat kk = (GLfloat)(iix * iix + iiy * iiy); // k^2
				GLfloat diff = 1.f / (1.f + visc * dt * kk);
				xterm.x *= diff;
				xterm.y *= diff;
				yterm.x *= diff;
				yterm.y *= diff;
				...
				if (kk > 0.f)
				{
					...
					GLfloat rkp = (iix * xterm.x + iiy * yterm.x);
					// Imaginary portion of velocity projection
					GLfloat ikp = (iix * xterm.y + iiy * yterm.y);
					xterm.x -= rkk * rkp * iix;
					xterm.y -= rkk * ikp * iix;
					yterm.x -= rkk * rkp * iiy;
					yterm.y -= rkk * ikp * iiy;
...
GLint dx, GLint pdx, GLint dy, GLint lb, size_t pitch)
{
	...
	if (gtidx < dx)
	{
		for (p = 0; p < lb; p++)
		{
			...
			if (fi < dy)
			{
				GLint fjr = fi * pdx + gtidx;
				vxterm = vx[fjr];
				vyterm = vy[fjr];

				// Normalize the result of the inverse FFT
				GLfloat scale = 1.f / (dx * dy);
				nvterm.x = vxterm * scale;
				nvterm.y = vyterm * scale;

				cData *fj = (cData *)((GLchar *)v + fi * pitch) + gtidx;
				*fj = nvterm;
			}
...
float dt, GLint lb, size_t pitch)
{
	...
	if (gtidx < dx)
	{
		for (p = 0; p < lb; p++)
		{
			...
			if (fi < dy)
			{
				...
				GLint xvi = ((GLint)(pterm.x * dx));
				GLint yvi = ((GLint)(pterm.y * dy));
				vterm = *((cData *)((GLchar *)v + yvi * pitch) + xvi);

				pterm.x += dt * vterm.x;
				pterm.x = pterm.x - (GLint)pterm.x;
				pterm.x += 1.f;
				pterm.x = pterm.x - (GLint)pterm.x;
				pterm.y += dt * vterm.y;
				pterm.y = pterm.y - (GLint)pterm.y;
				pterm.y += 1.f;
				pterm.y = pterm.y - (GLint)pterm.y;
...
void advectParticles(GLuint vbo, cData *v, GLint dx, GLint dy, GLfloat dt)
{
	...
	size_t num_bytes;
	cudaGraphicsResourceGetMappedPointer((GLvoid **)&p, &num_bytes,
		cuda_vbo_resource);
