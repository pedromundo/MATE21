...
/*
	...
	for (unsigned int i = 1; i <= pmodel->numtriangles; i++) {
		...
		f->ID		= iFace++;
		f->normal.x = pmodel->normals[pmodel->triangles[i].nindices[0]];
		f->normal.y = pmodel->normals[pmodel->triangles[i].nindices[1]];
		f->normal.z = pmodel->normals[pmodel->triangles[i].nindices[2]];		
